...
describe("fetched artwork tests with mocking", () => {
  //ensure we clear mocks between each test
  afterEach(() => {
    jest.resetAllMocks();
  });
  //   test("Should produce an error message if the user detects an empty string", () => {});
  //   test(
  //     "Should produce an timeout message if fufilling the request takes longer than 4 seconds"
  //   );
  test("Should fetch requested information by artist_title", async () => {
    ...
    const retrievedArtwork = await artChicagoApiController.getArtworks(
      req,
      next
    );
    expect(retrievedArtwork.data).toEqual(
