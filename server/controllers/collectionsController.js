...
collectionsController.readFavorites = async (req, res, next) => {
  ...
  try {
    ...
    const userFavoritesTableCTE =
      "WITH user_favorite_artworks AS (SELECT c.description, a.title, a.artist_title, a.medium, a.date_display, a.small_image_url, a.large_image_url FROM artworks a INNER JOIN collection_order co USING (artwork_id) INNER JOIN collections c USING(collection_id) WHERE c.id=$1) SELECT * FROM user_favorite_artworks;";
