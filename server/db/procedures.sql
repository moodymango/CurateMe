...
BEGIN
    RETURN QUERY
    SELECT id, first_name, password FROM users WHERE username=user;
    --output success message
...
CREATE OR REPLACE PROCEDURE locate_favorites_id(
    IN user INT, 
    OUT fav_collection_id INT)
AS $$
BEGIN
    --retrieve collection_id of user favorites collection
    SELECT id FROM collections INTO user_collection_id WHERE user_id=user AND title='favorites';
...
CREATE OR REPLACE PROCEDURE viewFavorites(IN user INT)
AS $$
DECLARE
    user_collection_id INT;
BEGIN
    CALL locate_favorites_id(user) INTO user_collection_id;
...
CREATE OR REPLACE PROCEDURE favorite_artworks(
    IN user INT, 
    IN artworkID INT, 
    IN artwork_title VARCHAR(150), 
    IN artist VARCHAR(150), 
    INT med VARCHAR(150), 
    IN date_dis VARCHAR(150), 
    IN imageID VARCHAR(150))
AS $$
DECLARE
    art_id INT;
...
BEGIN
    ...
    CALL locate_favorites_id(user) INTO fav_collection_id;
